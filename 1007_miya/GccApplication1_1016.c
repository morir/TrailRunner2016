...
/* 02:BIT_00010x */		{TRACE_R_STRAIGHT,	TRACE_R_STRAIGHT,	TRACE_R_STRAIGHT,	TRACE_R_STRAIGHT,	TRACE_R_STRAIGHT,	TRACE_L_TURN,		TRACE_STRAIGHT},
...
/* 08:BIT_01000x */		{TRACE_L_STRAIGHT,	TRACE_L_STRAIGHT,	TRACE_L_STRAIGHT,	TRACE_L_STRAIGHT,	TRACE_L_STRAIGHT,	TRACE_STRAIGHT,		TRACE_R_TURN},
	...
	while (1) {
		...
		if(currentTraceAction != previousTraceAction)
		{
			...
			if( previousTraceAction == TRACE_L_TURN && currentTraceAction == TRACE_STRAIGHT) {
				...
				currentTraceAction = TRACE_R_ROUND;//旋回復帰したとき中央センサーを通り過ぎてるので、反対側に曲げる
			}
			...
			if (previousTraceAction == TRACE_R_TURN && currentTraceAction == TRACE_STRAIGHT) {
				...
				_delay_ms(100);	// 100ms 逆回転を入力（強さと時間は調整必要）
				//モーターの速度0付近に
				currentTraceAction = TRACE_L_ROUND;//旋回復帰したとき中央センサーを通り過ぎてるので、反対側に曲げる
			}
